...
Both variants are specified in this document. The re-execution spec comes first because it is simpler and helps explain the progression to ZK: the core function being executed or proven is the same ([`verify_stateless_new_payload`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L359)), and the contract patterns (state management, messaging, anchoring) are shared. The ZK spec then shows what changes when re-execution is replaced by proof verification.

## Design principles

The core principle is to re-use as many L1 components as possible. L2 operators run the same proving infrastructure as L1 provers: they prove the same program ([`verify_stateless_new_payload`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L359)) with the same keys. L1 nodes verify L2 proofs using the same [EIP-8025](https://eips.ethereum.org/EIPS/eip-8025) infrastructure they use for L1 block proofs. In the re-execution variant, L1 validators run the same stateless validation function directly.
...
Both variants execute or prove the same function ([`verify_stateless_new_payload`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L359)) and therefore share the same L2 block structure. The following tables describe how native rollup blocks map to the standard spec types.
...
The unconstrained fields (`fee_recipient`, `prev_randao`, `parent_beacon_block_root`) correspond to the [`PayloadAttributes`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/execution_engine/types.py#L87) that on L1 are trusted to come from the consensus layer. The EL never validates them; it accepts whatever the CL provides. Since native rollups have no CL, these become free inputs for the operator. `timestamp` is also CL-provided on L1 but additionally constrained by the EL (`> parent_header.timestamp`).

### StatelessInput

[`StatelessInput`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L183): input to `verify_stateless_new_payload`.

| Field | Expected source | Notes |
|-------|-----------------|-------|
| `new_payload_request` | see below | |
| `witness` | calldata (re-execution) / offchain (ZK) | [`ExecutionWitness`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L34): MPT node preimages, contract bytecodes, ancestor headers |
| `chain_config` | storage (`chain_id`) | [`ChainConfig`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L172): L2 chain configuration |
| `public_keys` | calldata (re-execution) / offchain (ZK) | Pre-recovered ECDSA keys to avoid expensive recovery in the ZK circuit |

### NewPayloadRequest

[`NewPayloadRequest`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/execution_engine/types.py#L64) (inside `StatelessInput`):
...
[`ExecutionPayload`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/execution_engine/types.py#L29) (inside `NewPayloadRequest`):
...
The ethrex project implements a version of this approach ([PR #6186](https://github.com/lambdaclass/ethrex/pull/6186)). Our spec follows the same pattern but wraps the standard [`verify_stateless_new_payload`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L359) directly, rather than using a custom `apply_body` variant with individual ABI-encoded fields.

### The EXECUTE precompile

The `EXECUTE` precompile is the precompile equivalent of [`run_stateless_guest`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless_guest.py#L75), the ZK guest entry point that takes SSZ-serialized input, runs `verify_stateless_new_payload`, and returns SSZ-serialized output. The precompile adds L2-specific preprocessing (fixed-field checks) before calling the same function. The pseudocode uses types and functions from the [execution-specs](https://github.com/ethereum/execution-specs/tree/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam):
...
**Input:** The precompile reads its input from `evm.message.data`, which contains an SSZ-serialized [`StatelessInput`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L183), decoded via [`deserialize_stateless_input`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless_guest.py#L36).

**Output:**

- **On success:** `evm.output` contains the SSZ-serialized [`StatelessValidationResult`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L214), encoded via [`serialize_stateless_output`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless_guest.py#L28). This includes `new_payload_request_root`, `successful_validation`, and `chain_config` (with `chain_id`).
...
The specification follows the [stateless execution model](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py) from the execution-specs, the [Block-in-Blobs (EIP-8142)](https://eips.ethereum.org/EIPS/eip-8142) pattern for transaction data availability, and the [EIP-8025](https://eips.ethereum.org/EIPS/eip-8025) proof validation infrastructure from the consensus-specs.

The flow:

1. The **rollup operator** builds an L2 block and generates an execution proof by proving [`verify_stateless_new_payload`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L359), the same program that L1 provers prove for L1 blocks.
...
The ZK variant builds directly on the re-execution spec. The core function being verified is the same: [`verify_stateless_new_payload`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L359). What changes is *how* that function is verified and *where* the data lives.
...
- **Witness and public keys**: in re-execution, the [`ExecutionWitness`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L34) (trie node preimages, contract codes, ancestor headers) and pre-recovered public keys are passed as calldata because the EL needs them to re-execute. In ZK, neither is needed onchain: the prover uses them offchain to generate the proof, and they are not posted to L1.
...
**EL: transaction decoding and validation.** A new transaction type (e.g. `PROOF_TX_TYPE = 0x05`) is added to [`transactions.py`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/transactions.py#L270). The `ProofCarryingTransaction` class extends [`BlobTransaction`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/transactions.py#L270) with `validation_result_root: Hash32`. The [signing hash](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/transactions.py#L886) includes `validation_result_root`, so the sender commits to which L2 block is being proven.

[`check_transaction`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/fork.py#L496) applies the same validation as blob transactions (blob count, version byte, `max_fee_per_blob_gas >= blob_gas_price`, balance coverage including blob gas) plus:
...
**EL: transaction environment.** [`TransactionEnvironment`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/vm/__init__.py#L111) is extended with `validation_result_root: Hash32` (set from the transaction in [`process_transaction`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/fork.py#L958), or `bytes32(0)` for non-proof-carrying txs). This is what the `PROOFROOT` opcode reads, mirroring how [`blob_versioned_hashes`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/vm/instructions/environment.py#L558) in `TransactionEnvironment` is what `BLOBHASH` reads.

**EL: engine API.** [`is_valid_versioned_hashes`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/execution_engine/new_payload.py#L50) validates that versioned hashes from the CL match blob transaction hashes in the payload. It would need to be updated to also extract `blob_versioned_hashes` from `ProofCarryingTransaction` instances (currently it only handles `BlobTransaction`). Beyond this, no additional engine API changes are needed: the EL does not see or validate the proof, just as it does not see blob contents.

**CL: proof validation.** The consensus layer extracts the `execution_proof` from the proof-carrying transaction's sidecar and validates it using [`proof_engine.verify_execution_proof`](https://github.com/ethereum/consensus-specs/blob/a84880a47a88700d8dfa451c2a7cd4b3f309bd0d/specs/_features/eip8025/proof-engine.md#new-verify_execution_proof). Unlike L1 block proofs, which are delivered as [`SignedExecutionProof`](https://github.com/ethereum/consensus-specs/blob/a84880a47a88700d8dfa451c2a7cd4b3f309bd0d/specs/_features/eip8025/beacon-chain.md#new-signedexecutionproof) messages signed by active validators and processed via [`process_execution_proof`](https://github.com/ethereum/consensus-specs/blob/a84880a47a88700d8dfa451c2a7cd4b3f309bd0d/specs/_features/eip8025/beacon-chain.md#new-process_execution_proof), L2 proofs are delivered via the transaction sidecar and do not require a validator signature — the CL calls `verify_execution_proof` directly. The proof's public output must match the `validation_result_root` declared in the transaction body. If the proof is invalid, the L1 block is rejected, analogous to how an invalid KZG proof in a blob sidecar invalidates the block.
...
An L2 execution proof uses the same [`ExecutionProof`](https://github.com/ethereum/consensus-specs/blob/a84880a47a88700d8dfa451c2a7cd4b3f309bd0d/specs/_features/eip8025/beacon-chain.md#new-executionproof) structure as an L1 block proof. It proves that [`verify_stateless_new_payload`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L359) succeeded for the L2 block. The proof's public output is the full [`StatelessValidationResult`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L214):

- `new_payload_request_root`: the root of the [`NewPayloadRequest`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/execution_engine/types.py#L64) for that L2 block, computed via [`compute_new_payload_request_root`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L246)
- `successful_validation`: whether the state transition succeeded
- `chain_config`: the [`ChainConfig`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L172) (includes `chain_id`) used during execution (see [PR #2342](https://github.com/ethereum/execution-specs/pull/2342))

The `validation_result_root` declared in the proof-carrying transaction is a hash of this full `StatelessValidationResult`. Unlike L1 proofs, which are gossipped as [`SignedExecutionProof`](https://github.com/ethereum/consensus-specs/blob/a84880a47a88700d8dfa451c2a7cd4b3f309bd0d/specs/_features/eip8025/beacon-chain.md#new-signedexecutionproof) messages signed by active validators, L2 proofs are delivered via the proof-carrying transaction sidecar and do not require a validator signature. The CL validates them using the same [`proof_engine.verify_execution_proof`](https://github.com/ethereum/consensus-specs/blob/a84880a47a88700d8dfa451c2a7cd4b3f309bd0d/specs/_features/eip8025/proof-engine.md#new-verify_execution_proof) function (see [Transaction processing](#transaction-processing)).
...
1. **Compute `new_payload_request_root`** from block header fields via [`compute_new_payload_request_root`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L246). The hashing scheme is SSZ `hash_tree_root` over the [`SszNewPayloadRequest`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless_ssz.py#L177) container (see [`stateless_ssz.py`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless_ssz.py)). The contract only has header-level data (roots and scalar fields, not full transaction lists), so the scheme must allow reconstruction from [`NewPayloadRequestHeader`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L69) fields, which satisfies `hash_tree_root(execution_payload_header) == hash_tree_root(execution_payload)`.

2. **Hash the full `StatelessValidationResult`**: compute `hash_tree_root` of the [`SszStatelessValidationResult`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless_ssz.py#L238) containing `new_payload_request_root` (from step 1), `successful_validation = true`, and `chain_config` (with `chain_id` from storage). The result is the `validation_result_root`.
...
1. **N L2 execution proofs** (generated by rollup operators): each proves [`verify_stateless_new_payload`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L359) for one L2 block. Carried in proof-carrying transaction sidecars and validated by the CL independently.
...
9. **FOCIL / inclusion-list enforcement**: For [forced transactions](./forced_transactions.md) to be enforceable via the proof rather than via trie-membership checks, the proof's public output would need to expose an `is_inclusion_list_satisfied` flag analogous to [EIP-7805](https://eips.ethereum.org/EIPS/eip-7805)'s `block_output.is_inclusion_list_satisfied`. Today that field is an `apply_body` local that escapes via the engine API to the CL, and is never threaded into [`StatelessValidationResult`](https://github.com/ethereum/execution-specs/blob/02c6c2510916e470f2c1e5191589212ca75d4948/src/ethereum/forks/amsterdam/stateless.py#L214) because L1's chosen FOCIL enforcement is CL-side (attestation weights), not proof-side. Since native rollups reuse the L1 stateless surface verbatim (same `StatelessInput`, same `verify_stateless_new_payload`, same `StatelessValidationResult`), exposing the flag in the proof's public output is necessarily an L1-side change: every L1 block proof would publish it. Resolving this depends on whether L1 chooses to surface FOCIL state through the proof at all; if not, native rollups have to enforce inclusion via a different mechanism (e.g., trie-membership checks against `transactions_root` in the rollup contract, which is significantly more expensive).
